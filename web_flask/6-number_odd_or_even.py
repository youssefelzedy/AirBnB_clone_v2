...
@skill_app.route('/number_odd_or_even/<int:n>', strict_slashes=False)
def even_or_odd(n):
    """number integer message"""
    if n % 2 == 0:
        state = "{} is even".format(n)
    else:
        state = "{} is odd".format(n)
    return render_template('6-number_odd_or_even.html', state=state)
